...
Four bundles go through both: `Jamin.component` (AU), `Jamin.vst3`, `Jamin.app` (standalone) and
`Jamin MIDI FX.component`.

---

## The short version

This repository's secrets are already set. If they ever need re-checking or rotating, one script
verifies both credentials and pushes every secret:
...
That builds universal, signs, runs `auval` and `pluginval` **against the signed bundles**, runs
the installed host cycles, notarises, staples, packages and publishes a GitHub release.

The tag has to agree with `package.json` and `project(JaminNative VERSION …)` in
`native/CMakeLists.txt`; the `Music` job checks this on tag refs and fails the run if they
...
```
gh workflow run 'Plugins' --repo TonyGermaneri/jamin -f notarize=true
```
...
```
npm run build                      # both the page and the compiler — see below
...
native/tools/macos-notarize.sh native/build/plugin dist
```
...
`npm run build` rather than `npm run build:page`: the second builds only the page and Vite empties
the output directory on its way, which takes `jamin-compile.js` with it. The bundle then looks
perfectly correct and plays nothing at all. That is a hard build failure now — see
`native/cmake/WebAssets.cmake` — but it is worth knowing why.

---

## What "it worked" looks like

`macos-sign.sh` should report, for each of the four bundles, `valid on disk` and `satisfies its
...
```
codesign -dvvv native/build/plugin/JaminInstrument_artefacts/RelWithDebInfo/AU/Jamin.component \
...
- `flags=0x10000(runtime)` — the hardened runtime. Without it, notarisation is rejected twenty
  minutes later inside a JSON log. jamin needs no entitlements alongside it: QuickJS is an
  interpreter and never makes executable memory, so the hardened runtime has nothing to object to.
