...
instrument="$plugins/JaminInstrument_artefacts/$config"
midifx="$plugins/JaminMidiFx_artefacts/$config"

bundles=()
for candidate in "$instrument/AU/Jamin.component" \
                 "$instrument/VST3/Jamin.vst3" \
                 "$instrument/Standalone/Jamin.app" \
                 "$midifx/AU/Jamin MIDI FX.component"; do
...
# and four of them in series is four times the wait for the same answer.
work="$(mktemp -d "${TMPDIR:-/tmp}/jamin-notarize-XXXXXX")"
archive="$work/jamin.zip"
